...
class Document(Base):
    ...
    file_size = Column(String(100))  # Changed to string to match document_processor
    upload_date = Column(DateTime(timezone=True), server_default=func.now())
    processed = Column(Boolean, default=False)
    processing_error = Column(Text)
    status = Column(String(50), default="uploaded")  # Added status field
    ...
    vector_entries = relationship("VectorEntry", back_populates="document", cascade="all, delete-orphan")
