// Package extraction implements the PDF extraction pipeline: PDF type
// detection, text/table/metadata extraction, optional Gemini table markdown,
// optional OCR, and the structured JSON output shape the frontend consumes.
//
...
// and produce the artifacts the frontend needs.
package extraction

// StructuredContent is the JSON stored in extractions.content and returned by
// GET /api/documents/{id}/content. Field names and shape match what
...
// Page is one entry of pages[] in the output.
type Page struct {
